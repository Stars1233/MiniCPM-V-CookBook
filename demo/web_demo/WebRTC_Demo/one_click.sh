...
PYTHON_CMD="${PYTHON_CMD:-/Users/airlsyn/works/tools/miniconda3/envs/py311/bin/python}"
...
check_port() {
    ...
    # Method 4: direct connection attempt
...
start_livekit() {
    ...
    if is_running "$LIVEKIT_PID"; then
        warn "LiveKit is already running (PID: $(cat "$LIVEKIT_PID"))"
        return 0
    fi

    if check_port "$LIVEKIT_PORT"; then
        err "Port $LIVEKIT_PORT is already in use, please free it first:"
        lsof -i :"$LIVEKIT_PORT" 2>/dev/null || true
        return 1
    fi
...
start_backend() {
    ...
    if is_running "$BACKEND_PID"; then
        warn "Backend is already running (PID: $(cat "$BACKEND_PID"))"
        return 0
    fi

    if check_port "$BACKEND_PORT"; then
        err "Port $BACKEND_PORT is already in use, please free it first:"
...
start_cpp_server() {
    ...
    if is_running "$CPP_SERVER_PID"; then
        warn "C++ inference service is already running (PID: $(cat "$CPP_SERVER_PID"))"
        return 0
    fi

    if check_port "$CPP_SERVER_PORT"; then
        err "Port $CPP_SERVER_PORT is already in use, please free it first:"
        lsof -i :"$CPP_SERVER_PORT" 2>/dev/null || true
        return 1
    fi
...
start_frontend() {
    ...
    if is_running "$FRONTEND_PID"; then
        warn "Frontend is already running (PID: $(cat "$FRONTEND_PID"))"
        return 0
    fi

    if check_port "$FRONTEND_PORT"; then
        err "Port $FRONTEND_PORT is already in use, please free it first:"
        lsof -i :"$FRONTEND_PORT" 2>/dev/null || true
        return 1
    fi
    ...
    if [[ "$FRONTEND_MODE" == "prod" ]]; then
        ...
        if [[ ! -d "$FRONTEND_DIR/dist" ]] || [[ "$FORCE_BUILD" == "1" ]]; then
